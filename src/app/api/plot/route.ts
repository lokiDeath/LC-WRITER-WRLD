...
export async function POST(req: NextRequest) {
  ...
  const { novelId, title, type, status, importance, description } = body
  if (!novelId || !title) return NextResponse.json({ error: 'novelId, title required.' }, { status: 400 })
  const novel = await db.novel.findUnique({ where: { id: novelId } })
  if (!novel || novel.authorId !== user.id) return NextResponse.json({ error: 'Not found.' }, { status: 404 })
  const item = await db.plotThread.create({
